...
The format of the `data` byte string is defined by the wallet vendor. The most important consideration during an update is applying the desired mutation correctly. Ensure that no other parts of the configuration are accidentally changed or discarded.
...
Keyspace enables you to keep your wallet's configuration in sync across different chains. Syncing is optional, and is mainly recommended for Ethereum rollups. Even on chains where syncing is disabled, Keyspace's state-based configuration management helps you develop features that can replay their configuration changes on other chains without any feature-specific syncing work.

```solidity
function setConfig(ConfigLib.Config calldata newConfig, bytes calldata authorizationProof) {
    // Updates the wallet's configuration on the master chain.
}
